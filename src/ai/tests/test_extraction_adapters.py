...
def test_read_pdf_file_extracts_text_per_page(monkeypatch):
    """
    Mocks pdfplumber.open rather than generating a real PDF (no PDF-writing
    library is a project dependency) - this tests read_pdf_file()'s own
    logic (one row per page, blank pages skipped, PAGE_TEXT_HEADER as the
    single column) independent of pdfplumber's own extraction correctness,
    ...
    class FakePdf:
        pages = [FakePage("Invoice total 45.00 JOD"), FakePage("   "), FakePage(None)]
    ...
    monkeypatch.setattr(pdf_adapter_module.pdfplumber, "open", lambda path: FakePdf())
