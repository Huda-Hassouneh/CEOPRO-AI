...
def test_extract_and_store_news_records_no_pending_rows(fake_conn):
    with patch.object(pipeline.data_access, "load_pending_news_records", return_value=[]):
...
def test_extract_and_store_news_records_persists_and_marks_processed(fake_conn):
    ...
    with patch.object(pipeline.data_access, "load_pending_news_records", return_value=records), \
         patch.object(pipeline, "extract_entities", return_value=entities) as mock_extract, \
    ...
    mock_extract.assert_any_call(records[0]["body_text"], tenant_id="tenant-1", redis_client=None, conn=fake_conn)
    mock_mark.assert_any_call(fake_conn, "n1", "Processed")
    mock_mark.assert_any_call(fake_conn, "n2", "Processed")
    fake_conn.commit.assert_called_once()


def test_extract_and_store_news_records_passes_through_redis_client(fake_conn):
    """redis_client is caller-injected (same convention as ingestion_pipeline.py/rag's minio_client), never constructed here."""
    records = [_news("n1")]
    fake_redis = MagicMock()

    with patch.object(pipeline.data_access, "load_pending_news_records", return_value=records), \
         patch.object(pipeline, "extract_entities", return_value=[]) as mock_extract, \
         patch.object(pipeline.evidence, "insert_extracted_entities", return_value=[]), \
         patch.object(pipeline.data_access, "mark_news_record_status"):
        pipeline.extract_and_store_news_records(fake_conn, "tenant-1", redis_client=fake_redis)

    mock_extract.assert_called_once_with(records[0]["body_text"], tenant_id="tenant-1", redis_client=fake_redis, conn=fake_conn)
...
def test_extract_and_store_social_mentions_persists_and_marks_processed(fake_conn):
    ...
    with patch.object(pipeline.data_access, "load_pending_social_mentions", return_value=records), \
