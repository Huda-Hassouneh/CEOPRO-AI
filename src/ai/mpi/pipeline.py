...
def _usable_reviews(reviews: list) -> list:
    """
    A review with no effective_date can't get a recency weight, so it's
    excluded from scoring entirely - not down-weighted, excluded. review_date
    is NOT NULL in the schema, so in practice this only matters defensively;
    filtered once here so every downstream
...
def get_subject_mpi(
    ...
    if result is None:
        # Either no reviews at all, or every review had no effective_date and
        # got filtered out - same UNKNOWN outcome either way: nothing usable
        # to score.
