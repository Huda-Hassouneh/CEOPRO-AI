...
def generate_platform_help_summary(conn=None, tenant_id: str = None) -> str:
    """
    Closes a real, direct product gap: the chatbot could only ever answer
    questions about a tenant's own business data (sales, competitors,
    pricing, forecasts) - a question about the PLATFORM itself ("how do I
    add a product", "how do I connect my database") had nothing to
    retrieve, ever, because no platform-help content existed anywhere in
    this codebase. Every capability named below is a real, currently
    implemented endpoint (src/ai/main.py) - nothing here is aspirational
    or describes a UI screen, since no frontend exists in this codebase
    yet to describe (PENDING_ACTIONS.md's own onboarding-flow note) -
    written for the same audience SYSTEM_PROMPT targets (an ordinary shop
    owner), not as API documentation.

    conn/tenant_id accepted (unused) only so this fits _GENERATORS' shared
    (conn, tenant_id) -> str call signature alongside every other slot
    generator, and can be regenerated by the exact same scheduled call
    (regenerate_all_structured_summaries(), already run automatically
    after every scrape/forecast event) without a special case.

    Also carries the platform's own identity/mission (source:
    MASTER_SPEC_v4.md Section 3, "SYSTEM DEFINITION" - the real,
    authoritative project spec, not marketing copy invented for this
    prompt) - closes the other half of the same product gap: a user
    asking "who are you" or "what is this platform" used to have nothing
    grounded to answer from either, same as "how do I use this".
    """
    return (
        "Who CEOPRO AI is (source: MASTER_SPEC_v4.md, Section 3 - System Definition) - answer "
        "a question about the platform's own identity or purpose using this, the same way "
        "you'd answer a question about the tenant's own business data: CEOPRO AI is an "
        "AI-powered business intelligence platform built for small and medium-sized "
        "businesses, made to work across many different countries (not built for just one "
        "country or region) rather than one specific market. It takes a business's own data "
        "together with real market and competitor information, and turns both into clear "
        "insights, forecasts, and recommendations - shown on a dashboard and through this "
        "chat - so a business owner can make decisions with real information instead of "
        "guesswork.\n\n"
        "How to use CEOPRO AI (source: this platform's own real, implemented features "
        "- src/ai/main.py) - answer a question about the platform itself using this, "
        "the same way you'd answer a question about the tenant's own business data:\n\n"
        "Getting your data in, two ways:\n"
        "1. Upload a file - a spreadsheet or PDF of your sales/inventory (.csv, .xlsx, .xlsm, or "
        ".pdf) is read automatically: every row is validated and typed, nothing is silently "
        "dropped even if a row fails, and successfully read rows become real products and sales "
        "records right away.\n"
        "2. Connect your own system directly - your own database, or your own POS/ERP/e-commerce "
        "system's API, can be connected once so new sales/inventory sync in automatically on a "
        "schedule afterward, instead of uploading a file every time. A quick status check is "
        "available to see whether a data connection exists yet at all.\n\n"
        "What happens automatically once you have real products in the system:\n"
        "- Competitor discovery: real, public competitor storefronts for your kind of business are "
        "found and tracked (never Facebook/Instagram/TikTok directly, and never a site whose own "
        "rules disallow it - only what's genuinely public and permitted).\n"
        "- Demand forecasting: a 7-day demand forecast is computed per product from its own real "
        "sales history.\n"
        "- Price recommendations: your own price is compared against real, currently tracked "
        "competitor prices, with a suggestion to raise, lower, or hold - never below cost, never an "
        "extreme single-step change, and never a guess when there's no real competitor price to "
        "compare against.\n"
        "- Sentiment: real competitor reviews, where found, are read and summarized as positive/"
        "neutral/negative, in plain language, not raw scores.\n\n"
        "Where to see all this at a glance: a dashboard view exists with top KPIs (revenue, units "
        "sold, transaction growth, competitors tracked), the top few actionable recommendations "
        "right now, a total forecast-demand summary, and a competitor price-positioning comparison "
        "per product.\n\n"
        "This chat itself: ask anything about your own real data - sales, competitors, pricing, "
        "sentiment, forecasts - in English, Arabic, or a natural mix, and it answers grounded only "
        "in your own real, ingested data, never inventing a number or a competitor that isn't "
        "actually there."
    )
