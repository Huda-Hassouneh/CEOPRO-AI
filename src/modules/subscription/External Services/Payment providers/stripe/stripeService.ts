...
import { toStripeMinorUnits } from "../../../../../utils/currency.js";
...
export async function createCustomer(
  ...
  try {
    ...
    /*
     * Legacy fallback is permitted only when no tenant context exists.
     *
     * If tenantId is present it is authoritative: falling back to userId
     * could attach the same Stripe Customer to two tenants when a user is a
...
export async function createPromoCode(
  ...
  try {
    ...
    if (!currency) {
      throw new Error("Currency is required for a fixed-amount Stripe coupon");
    }

    // Application amounts are expressed in major units; convert exactly once
    // at the Stripe adapter boundary.
    return await stripe.coupons.create({
      amount_off: toStripeMinorUnits(amount, currency),
...
export async function retrievePromoCode(
  ...
  if (!couponId) throw new Error("Stripe coupon ID is required");
  const coupon = await stripe.coupons.retrieve(couponId);
  if ((coupon as any).deleted) throw new Error("Stripe coupon is deleted");
  return coupon as Stripe.Coupon;
}
...
export async function createCheckoutSession(
  ...
  try {
    return await stripe.checkout.sessions.create({
       ...
       * Otherwise Checkout creates/associates one from email.
       */
      ...(customerId
        ? {
            customer: customerId
      ...
      /*
       * Require the user to provide a payment method
       * even when the subscription starts with a free trial.
       *
       * This means the card is available for automatic
       * charging when the trial ends.
       */
      payment_method_collection: "always",

      /*
       * Configuration for the Subscription that Checkout creates.
       */
      subscription_data: {
        metadata: {
          tenantId
        ...
        /*
         * Only configure a trial when the plan actually
         * has a positive trial period.
         */
        ...(trialPeriodDays && trialPeriodDays > 0
          ? {
               ...
               * IMPORTANT:
               *
               * Do NOT use "pause" here if your application
               * later converts the subscription into a
               * Subscription Schedule.
               *
               * Stripe rejects schedule creation from a
               * subscription configured to pause at trial end.
               *
               * Because payment_method_collection = "always",
               * this should normally never be reached anyway.
               *
               * If the payment method somehow disappears before
               * trial end, canceling is the clean fallback.
               */
              trial_settings: {
                end_behavior: {
...
async function ensureScheduleCompatibleTrialSettings(
...
export async function updateSubscription(data: {
  ...
  if (action === "upgrade") {
    /*
     * If a future downgrade is already scheduled,
     * release it before performing the upgrade.
     */
     ...
     * - Customer keeps the ORIGINAL trial end.
     * - No proration.
     * - No charge now.
     * - At trial end Stripe charges the FULL new-plan price.
     * - Changing plans never grants a new trial.
     */
    if (subscription.status === "trialing") {
      if (!subscription.trial_end) {
        throw new Error(
          `Stripe subscription ${paymentProviderSubscriptionId} is trialing but has no trial_end.`
        );
      }
   ...
   */
  if (!scheduleId) {
    await ensureScheduleCompatibleTrialSettings(paymentProviderSubscriptionId);
