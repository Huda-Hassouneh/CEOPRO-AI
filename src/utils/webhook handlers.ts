...
export async function handleSubscriptionScheduleEnded(
  ...
  /*
   * Reconcile the authoritative Stripe subscription before clearing the local
   * scheduled marker. This makes the terminal schedule event safe even if the
   * corresponding customer.subscription.updated webhook is delayed or arrives
   * out of order.
   */
  const stripeSubscription =
    await stripeService.stripe.subscriptions.retrieve(stripeSubscriptionId);
  ...
  /*
   * NOW it is safe to clear local pending schedule data. At this point planId
   * has already been synchronized from Stripe's currently active Price.
   */
