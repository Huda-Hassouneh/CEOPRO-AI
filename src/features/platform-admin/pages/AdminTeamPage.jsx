...
export function AdminTeamPage() {
  ...
  return <><Heading title="adminTeam" description="adminTeamDescription" actions={admin.can('adminTeam.invite') && <Button leadingIcon={<UserPlus size={17} />} onClick={() => { setEmail(''); setRole('VIEWER'); setDialog({ type: 'invite' }); }}>{t('invite')}</Button>} /><AdminTable title={t('adminTeam')} query={query} table={table} filters={[{ key: 'role', options: PLATFORM_ROLES }, { key: 'status', options: ['active', 'inactive', 'pending'] }]} columns={[{ key: 'name', render: m => <Identity name={m.name} email={m.email} /> }, { key: 'role', render: m => <Badge value={m.role} /> }, { key: 'status', render: m => <Badge value={m.status} /> }, { key: 'createdAt', label: 'joined', render: m => <DateValue value={m.createdAt} /> }]} rowAction={admin.can('adminTeam.roles.manage') ? member => <Button variant="ghost" size="sm" onClick={() => start(member)}>{t('memberActions')}</Button> : undefined} />
