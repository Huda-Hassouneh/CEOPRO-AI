...
import {
  PREVIEW_PLANS,
  replacePreviewPlan
} from "../../../shared/catalog/planCatalog.js";
...
export const getPreviewPrincipal = () =>
  role
    ? {
        ...db["admin-team"].find((m) => m.id === "preview-admin"),
        role,
        roleKey: role,
...
function audit(action, domain, id, before, after, companyId) {
  db["audit-logs"].unshift({
    ...
    targetType: domain,
    target: id,
    companyId: companyId || (domain === "companies" ? id : null),
    result: "success",
...
export function paginate(rows, query = {}) {
  let result = rows.filter(
    (row) =>
      (!query.q ||
        [row.name, row.email, row.company, row.actor, row.target].some((v) =>
          String(v || "")
            .toLowerCase()
      ...
      [
        ...
        "action",
        "targetType"
      ].every((key) => !query[key] || String(row[key]) === query[key]) &&
