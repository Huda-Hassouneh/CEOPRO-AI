...
export function TeamSettings({ query, permissions, companyId, locale, t, onNotice }) {
  ...
  const inviteButton = permissions.canInviteMembers ? <Button size="sm" leadingIcon={<UserPlus size={14} />} onClick={() => setInviteOpen(true)}>{t('settings.team.invite.action')}</Button> : null;
  return <><SettingsSection title={t('settings.team.title')} subtitle={t('settings.team.subtitle')} actions={inviteButton}>
    ...
    {query.data.available && members.length === 1 && permissions.canInviteMembers && <div className="settings-team-first"><p>{t('settings.team.firstTeam')}</p><Button size="sm" variant="outline" onClick={() => setInviteOpen(true)}>{t('settings.team.invite.action')}</Button></div>}
  ...
  </SettingsSection><InviteMemberModal open={inviteOpen && permissions.canInviteMembers} capabilities={capabilities} permissions={permissions} onClose={() => setInviteOpen(false)} members={members} invitations={invitations} companyId={companyId} t={t} onNotice={onNotice} /></>;
}
