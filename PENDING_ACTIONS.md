...
| 4 | `reviews`/`news_record`/`social_mention`/`extracted_entity`/`sentiment_result` tables don't exist | Infra/DB | Rule-based NER logic itself is now built and tested (`src/ai/extraction/`, spec §15) but has no table to write results to; `reviews`/`news_record`/`social_mention` also still block sentiment analysis (spec §16) entirely, since there's no raw text to run it on either | 🟡 Open — AI/ML side (extraction logic) is ready and waiting on `extracted_entity`; sentiment analysis hasn't started (needs both the source-text tables and a heavier model) |
