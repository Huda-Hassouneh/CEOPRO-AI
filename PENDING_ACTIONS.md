...
| 4 | `reviews`/`news_record`/`social_mention`/`extracted_entity`/`sentiment_results` tables don't exist | Infra/DB | NER persistence (spec §15) and sentiment analysis (spec §16) | 🟡 Partially resolved 2026-08-07 — `reviews` and `sentiment_results` both landed, which unblocked sentiment analysis entirely (source text + a place to write results); `src/ai/sentiment/` is now built and tested (see `AI_PROGRESS.md`). `extracted_entity`, `news_record`, `social_mention` are still missing, so NER persistence and news/social-based market intelligence remain blocked. |
...
| 19 | `src/infrastructure/database/seed_demo_data.py` (new, added 2026-08-07) does not run against the actual schema — will crash immediately, then fail differently if that's fixed. Confirmed by reading the real `init_schema.sql` this session has been testing against all day: (a) `random.poissonvariate` is called for transaction quantities (line 78) — **that method doesn't exist anywhere in Python's `random` module** (confirmed: `AttributeError: module 'random' has no attribute 'poissonvariate'`), so the script raises before writing a single transaction row; (b) even past that, every INSERT uses column names that don't match the real schema — e.g. `companies (id, name, country, currency, ...)` vs. the actual `companies (tenant_id, business_name, country_code, primary_currency, ...)`, `products (id, ..., name, ..., base_price, ...)` vs. the actual `products (product_id, ..., product_name, ..., current_price, ...)`, and `demand_forecasts (id, ..., forecast_date, predicted_quantity, confidence_interval, ...)` vs. the actual `demand_forecasts (forecast_id, ..., forecast_target_date, expected_demand, confidence_range_lower, confidence_range_upper, model_version, ...)` (`model_version` is `NOT NULL` and never supplied); (c) `except`/`finally` reference `conn`/`cursor`, which are never bound if `psycopg2.connect()` itself throws, so a connection failure raises a confusing second error instead of the intended one; (d) same hardcoded-credential-fallback pattern already flagged for `watchdog.py` (#10) and `ai_consumer.py` — `pwd = os.getenv("POSTGRES_PASSWORD", "SecureProductionPassword2026")`. | Whoever owns `seed_demo_data.py` (Infra/DB) | Anyone running this script gets an immediate crash, not seeded demo data — currently unusable as-is | 🔴 Open, new 2026-08-07 |
