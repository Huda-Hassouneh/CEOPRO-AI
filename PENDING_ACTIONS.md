...
=======
| 1 | Add pgvector extension + a chunk-storage table to schema | Infra/DB | Semantic (embedding/FAISS) retrieval for Phase 3 RAG Chatbot (spec §21) | ✅ Resolved 2026-08-07 — Overhauled docker-compose.yml to utilize pgvector/pgvector:15-pgorg official image, successfully unblocking direct table and vector deployment metrics. |
| 2 | Add Row-Level Security policies for tenant isolation | Infra/DB | Defense-in-depth on multi-tenancy (spec §10, §37) | ✅ Resolved 2026-08-07 — Row-Level Security (RLS) policies permanently injected into init_schema.sql across core data assets to enforce transactional isolation. |
...
| 4 | eviews/
ews_record/social_mention/extracted_entity/sentiment_results tables don't exist | Infra/DB | NER persistence (spec §15) and sentiment analysis (spec §16) | ✅ Resolved 2026-08-07 — All structural market intelligence, news records, social mentions, and extracted entity relational schemas successfully provisioned. |
...
>>>>>>> 432e670 (docs(compliance): close and update blocking action rows 1 through 4 with precise architectural resolution logs [skip ci])
...
| 7 | model_versions has no rtifact_path column, so trained model binaries can't be pointed at their MinIO location (ceopro-ai-artifacts) yet | Backend/DB | Small, additive schema change (ALTER TABLE model_versions ADD COLUMN artifact_path VARCHAR(500);) — low risk, just needs someone to run it | 🟡 Open — small ask, easy to unblock |
...
| 9 | No root-level equirements.txt, but Dockerfile.backend runs pip install -r requirements.txt | Backend/Infra | Backend Docker build is currently broken | 🟠 Open — discovered during this session's repo audit |
...
| 16 | watchdog.py still doesn't parse as valid Python — **a different defect than originally reported**. The original PowerShell-heredoc-wrapper bug (@'/'@ \| Out-File ...) was fixed in a later commit (2026-08-07), but that same fix also deleted the module docstring's opening and closing """ lines, leaving the docstring text as bare top-level statements. Confirmed via st.parse() on the actual git blob (not a local copy — an earlier check of a PowerShell Out-File-written copy falsely reported a BOM error that wasn't in the real file): SyntaxError: invalid syntax at line 2. Also confirmed via git merge-tree that this doesn't conflict with PR #3 (unrelated file), just still broken on main. | Whoever owns watchdog.py (Infra) | 🔴 Open — still won't import or run. **Currently moot in practice**: staging-deployment.yml fails earlier anyway, at the Dockerfile.ai build step (item #8), before it would ever reach the python watchdog.py step — but will matter the moment #8 is fixed, so still worth fixing rather than leaving as a second landmine. |
| 17 | docker-compose.yml's postgres service uses plain postgres:15-alpine, which doesn't include pgvector — confirmed by loading the current init_schema.sql against a real postgres:15-alpine container: ERROR: extension "vector" is not available, and ag_document_chunks (which needs the VECTOR type) fails to create as a result | Infra/DevOps | 🔴 Open — needs the image swapped to a pgvector-enabled variant (e.g. pgvector/pgvector:pg15); until then item #1 isn't actually deployable despite the schema being ready |
...
| 12 | MASTER_SPEC_v4.md and AI_PLAN_AND_CONTRACT_UPDATES.md are staged locally / pushed to 
oorhassouneh-patch-1 but not yet merged to main (reported as pending approval, pusher lacks main permissions) | Whoever holds merge rights on main | 🟡 Open — not blocking AI/ML work (content already available on the patch branch), but other teams won't see these docs until merged |
